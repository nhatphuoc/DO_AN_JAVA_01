...
public class SlangDictionary {
    ...
    private static void createAndShowGUI() {
        ...
        JMenuItem slangOfTheDayItem = new JMenuItem("Slang of the Day");
        quizMenu.add(randomSlangQuizItem);
        quizMenu.add(randomDefinitionQuizItem);
        quizMenu.add(slangOfTheDayItem);
        ...
        editSlangItem.addActionListener(e -> {
            ...
            String definition = JOptionPane.showInputDialog(frame,
                    "Enter New Definition (use '|' for multiple meanings):", String.join("|", slangMap.get(slang)));
            if (definition != null) {
                List<String> definitionList = new ArrayList<>();
                Collections.addAll(definitionList, definition.split("\\|"));
                slangMap.put(slang, definitionList);
        ...
        randomSlangQuizItem.addActionListener(e -> {
			...
			List<String> correctDefinitions = slangMap.get(randomSlang);
			String correctDefinition = correctDefinitions.get(new Random().nextInt(correctDefinitions.size()));

			List<String> options = new ArrayList<>(correctDefinitions);
			while (options.size() < 4) {
				String randomSlangOption = keys.get(new Random().nextInt(keys.size()));
				List<String> definitionsOption = slangMap.get(randomSlangOption);
				String randomDefinition = definitionsOption.get(new Random().nextInt(definitionsOption.size()));
			...
			String message = "Which of the following is a definition of: " + randomSlang + "?";
			int answer = JOptionPane.showOptionDialog(frame, message, "Random Slang Quiz", JOptionPane.DEFAULT_OPTION,
					JOptionPane.INFORMATION_MESSAGE, null, options.toArray(), options.get(0));

			if (options.get(answer).equals(correctDefinition)) {
				JOptionPane.showMessageDialog(frame, "Correct!");
			} else {
				JOptionPane.showMessageDialog(frame, "Wrong! The correct answer was: " + correctDefinition);
			}
		});
        ...
        randomDefinitionQuizItem.addActionListener(e -> {
            ...
            int answer = JOptionPane.showOptionDialog(frame, message, "Random Definition Quiz",
                    JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null, options.toArray(),
                    options.get(0));
        ...
        slangOfTheDayItem.addActionListener(e -> {
            List<String> keys = new ArrayList<>(slangMap.keySet());
            if (keys.isEmpty()) {
                JOptionPane.showMessageDialog(frame, "No slang words available.");
                return;
            }
            String randomSlang = keys.get(new Random().nextInt(keys.size()));
            List<String> definitions = slangMap.get(randomSlang);
            ...
            JOptionPane.showMessageDialog(frame, "Slang of the Day: " + randomSlang + "\nMeaning: " + definition);
        });
